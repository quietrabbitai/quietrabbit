...
fn gate1_inputs(label: &str) -> G1Inputs {
    ...
    match label {
        ...
        "gate1::all_omit::tier2" => (
            track(vec![field("secret", "value", "personal", "omit", "omit")]),
            2,
            2,
            2,
            ol(),
        ...
        "gate1::summarize::personal::tier2" => (
            track(vec![field(
                "x",
                "SomeValue",
                "personal",
        ...
        "gate1::summarize::financial::tier2" => (
            track(vec![field(
                "x",
                "SomeValue",
                "financial",
        ...
        "gate1::ordering_interleaved_omits::tier2" => (
            track(vec![
        ...
        "gate1::fields_shared_accuracy::tier2" => (
            track(vec![
        ...
        l if l.starts_with("gate1::floor_clamp::") => {
            ...
            let eff: u8 = re_str
                ...
                .nth(1)
                .unwrap()
                .trim_start_matches("eff")
                .parse()
                .unwrap();
            let value = if policy == "range_only" {
                "50000"
            } else {
                "TestVal"
            };
            let snss = if policy == "range_only" {
                "financial"
            } else {
                "personal"
            };
...
fn nfd(s: &str) -> String {
    unicode_normalization::UnicodeNormalization::nfd(s).collect()
}
...
fn gate2_inputs(label: &str) -> G2Inputs {
    ...
    match label {
        "gate2::no_match" => ("The weather is nice today.".into(), alice_t(), None),
        "gate2::exact_match_ascii" => ("Hello Alice, how are you?".into(), alice_t(), None),
        "gate2::case_insensitive_ascii" => ("hello alice, how are you?".into(), alice_t(), None),
        "gate2::below_min_length_3" => (
            "The code is ABC here.".into(),
            track(vec![field("code", "ABC", "personal", "pass", "pass")]),
            None,
        ),
        "gate2::at_min_length_match" => (
            "The code is ABCD here.".into(),
            track(vec![field("code", "ABCD", "personal", "pass", "pass")]),
            None,
        ),
        ...
        "gate2::unicode_german_exact" => ("Lives on Stra\u{00df}e now.".into(), german_t(), None),
        "gate2::unicode_german_lower" => ("lives on stra\u{00df}e now.".into(), german_t(), None),
        "gate2::unicode_german_ss_upper" => ("LIVES ON STRASSE NOW.".into(), german_t(), None),

        "gate2::unicode_nfc_field_nfc_response" => {
            let v = nfc("caf\u{00e9}");
            (
                format!("We visited {v}."),
                track(vec![field("place", &v, "personal", "pass", "pass")]),
                None,
            )
        }
        "gate2::unicode_nfd_field_nfc_response" => {
            ...
            (
                ...
                track(vec![field("place", &vf, "personal", "pass", "pass")]),
                None,
            )
        }
        ...
        "gate2::overlap_value_repeated" => ("AliceAlice is the name.".into(), alice_t(), None),
        ...
        "gate2::disclosure_log_on_flag_tier2" => ("Hello Alice.".into(), alice_t(), None),
...
fn test_apply_abstraction_valid() {
    ...
    let vectors = root["apply_abstraction_valid"]
        .as_array()
...
fn test_apply_abstraction_invalid() {
    let root = load_json("gate1.json");
    let vectors = root["apply_abstraction_invalid"]
        .as_array()
...
async fn test_gate1_normal_paths() {
    ...
    for v in vectors {
        ...
        if label.contains("disclosure_log_failure") {
            step += 1;
            run += 1;
            continue;
        }
        ...
        .await
        .unwrap_or_else(|e| panic!("{label}: gate1 returned Err: {e}"));
        ...
        let expected_approved: Vec<(String, String)> = v["approved_fields"]
            ...
            .collect();
        let rust_approved: Vec<(String, String)> = result
            .approved_fields
            .iter()
        ...
        let expected_withheld: Vec<String> = v["withheld_fields"]
            ...
            .collect();
        assert_eq!(
            result.withheld_fields, expected_withheld,
        ...
        let expected_shared: Vec<String> = v["fields_shared"]
            ...
            .collect();
        assert_eq!(
            result.fields_shared, expected_shared,
        ...
        let expected_clamped: Vec<String> = v["floor_clamped_fields"]
            ...
            .collect();
        assert_eq!(
            result.floor_clamped_fields, expected_clamped,
        ...
        assert_eq!(
            logger.entry_count(),
            1,
            "Expected 1 disclosure log write [{label}], got {}",
            logger.entry_count()
        );
...
async fn test_gate1_disclosure_failure_tier1_nonfatal() {
    ...
    assert!(
        result.is_ok(),
        "Tier 1 disclosure failure must be non-fatal but got Err: {:?}",
        result.err()
    );
    assert_eq!(
        result.unwrap().disclosure_log_id,
        "",
...
async fn test_gate2() {
    ...
    for v in vectors {
        ...
        .await
        .unwrap_or_else(|e| panic!("{label}: gate2 returned Err: {e}"));

        assert_eq!(
            result.flagged, expected_flagged,
            "flagged MISMATCH [{label}]  rust={} python={}",
            result.flagged, expected_flagged
        );
        assert_eq!(
...
async fn test_gate2_t1_log_failure_nonfatal() {
    ...
    assert!(
        result.is_ok(),
        "T1 log failure must be non-fatal: got {:?}",
        result.err()
    );
    let r = result.unwrap();
    assert!(
    ...
    assert!(
        result2.is_err(),
        "T2+ log failure must be fatal: got Ok({:?})",
        result2.ok()
    );
}
...
async fn test_gate3() {
    ...
    for v in vectors {
        ...
        assert_eq!(
            result.approved, expected_approved,
